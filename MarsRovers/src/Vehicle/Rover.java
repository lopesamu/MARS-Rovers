...
public class Rover {
	...
	public Rover(Coordinate initialPosition, String initialDirection, String[] path, Plateau plateau,
			ArrayList<Rover> squad) {
	...
	public void finalPosition() {
		...
		for (int i = 0; i < this.path.length; i++) {
			if (this.path[i].equals("L")) {
				...
				if (finalDirection.equals("N")) {
					// check if the movement can take the robot off the platform or if it will
					// collide with another rover. If yes, cancel the move
					if (!this.isOffThePlateau(finalPositionX, finalPositionY + 1)
							&& !this.occurredCollision(finalPositionX, finalPositionY + 1)) {
						finalPositionY++;
					}
				} else if (finalDirection.equals("S")) {
					if (!this.isOffThePlateau(finalPositionX, finalPositionY - 1)
					...
					if (!this.isOffThePlateau(finalPositionX + 1, finalPositionY)
							&& !this.occurredCollision(finalPositionX + 1, finalPositionY)) {
						finalPositionX++;
					}
				} else if (finalDirection.equals("W")) {
					if (!this.isOffThePlateau(finalPositionX - 1, finalPositionY)
	...
	boolean isOffThePlateau(int x, int y) {
		if (x <= this.plateau.getUpperRight().getX() && x >= this.plateau.getLowerLeft().getX()
				&& y <= this.plateau.getUpperRight().getY() && y >= this.plateau.getLowerLeft().getY()) {
			return false;
		} else {
			return true;
		}
	...
	// checks whether a given move can cause the rover to collide with others who
	// have already made their move
	boolean occurredCollision(int x, int y) {
		// scroll through the list of squads. If the position (x, y) is equal to any
		// element in the list, return true
