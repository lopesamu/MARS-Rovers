...
class TestOutput {
	...
	// test case where the rover does not collide with another rover or leave the
	// plateau
	@Test
	void testNoColision() throws IOException {
		...
		for (int i = 1; i < input.getInputData().size() - 1; i += 2) {
			...
			Coordinate initialPosition = new Coordinate(input.getxInitialPositionRover(i),
					input.getyInitialPositionRover(i));
		...
		for (int i = 0; i < output.getR().size(); i++) {
			assertEquals(output.getR().get(i).getFinalPosition().getX() + " "
					+ output.getR().get(i).getFinalPosition().getY() + " " + output.getR().get(i).getFinalDirection(),
					expected[i]);
		}
	}
	...
	// test case where the rover might collide with another rover or leave the
	// plateau
	@Test
	void testColisionAndOutOfPlateau() throws IOException {
		...
		for (int i = 0; i < o.getR().size(); i++) {
			assertEquals(o.getR().get(i).getFinalPosition().getX() + " " + o.getR().get(i).getFinalPosition().getY()
					+ " " + o.getR().get(i).getFinalDirection(), expected[i]);
		}
	}
}
